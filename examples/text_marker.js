...
exports.makeRegexOrPattern = makeRegexOrPattern;
...
function makeRegexOrPattern(strings) {
...
function range(symbol, name) {
  ...
  var pattern = void 0;
  var setAttributes = options.setAttributes;
  ...
  return {
    ...
    onMatch: function onMatch(match) {
      ...
      if (typeof setAttributes === 'function') {
        var _attributes = setAttributes(match);
        rangeStart.attributes = _attributes;
        rangeEnd.attributes = _attributes;
      }
...
function keyword(symbol, name) {
  ...
  if (typeof symbol === 'string') {
    pattern = new RegExp('(^|\\s)(' + (0, _utils.escapeStringForRegex)(symbol) + ')(\\s|$)', 'gi');
  }

  if (symbol instanceof Array) {
    pattern = new RegExp('(^|\\s)(' + (0, _utils.makeRegexOrPattern)(symbol) + ')(\\s|$)', 'gi');
  }

  if (!pattern) {
    console.error('Cannot create a keyword token without a string or regex. Cannot use:', typeof symbol === 'undefined' ? 'undefined' : _typeof(symbol));
    return false;
  }
  return {
    pattern: pattern,
...
function block(delimiters) {
  ...
  return {
    ...
    onMatch: function onMatch(match) {
      ...
      if (typeof setAttributes === 'function') {
        var attributes = setAttributes(match);
...
module.exports = {"name":"text-marker","version":"0.10.2","description":"A lexical analyzer for parsing text","main":"dist/text_marker.js","repository":{"type":"git","url":"https://github.com/Paul-Guerra/text-marker.git"},"scripts":{"postinstall":"npm run mkdirs","mkdirs":"mkdir -p tmp/build/","rmdirs":"rm -rf tmp/","start":"npm run build && npm run webpack:watch & babel src/ --out-dir tmp/ --watch","compile":"npm run rmdirs && npm run mkdirs && babel src/ --out-dir tmp/","build":"npm run compile && webpack","webpack:watch":"webpack --watch","build:prod":"npm run compile && webpack --config ./webpack.prod.config.js","test":"jest","test:config":"jest --showConfig","lint":"eslint src/**/*.js"},"author":"","license":"MIT","devDependencies":{"babel-cli":"^6.24.1","babel-preset-es2015":"^6.24.1","eslint":"^4.3.0","eslint-config-airbnb":"^15.1.0","eslint-loader":"^1.9.0","eslint-plugin-babel":"^4.1.2","eslint-plugin-import":"^2.7.0","eslint-plugin-react":"^7.1.0","jest":"^20.0.4","uglifyjs-webpack-plugin":"^0.4.6","webpack":"^3.4.1","webpack-merge":"^4.1.0"},"dependencies":{}}
